...
if __name__ == '__main__':
    ...
    M: int = 101_000               # was 101_000
    length_measurement: int = 5   # was 5, reduces measurement set from 243 to 27
    switch_time = 20_000           # half‑way point. initially 20_000, 
    ...
    if measurement_to_label:
        nn_model = neural_network.MLPClassifier(random_state=random_state,
                                                max_iter=max_train_iter, warm_start=True,
                                                hidden_layer_sizes=hidden_layer_sizes)
        svm_model = svm.SVC(kernel=svm_kernel)
    else:
        nn_model = neural_network.MLPRegressor(random_state=random_state,warm_start=True,
                                               max_iter=max_train_iter,
                                               hidden_layer_sizes=hidden_layer_sizes)
        svm_model = SGDRegressor(loss='epsilon_insensitive', max_iter=1000,
                         tol=1e-3, warm_start=True, random_state=None)
    svm = BanditSklearnModel(window_size=300, action_set=game.action_set, measurement_set=game.measurement_set,
           raw_measurement=False, measurement_to_label=measurement_to_label, finite_measurement=True, policy_map=label_to_action,
          update_frequency=100, model=svm_model)
    svm.name = 'SVC_bandit'          # <-- add this line

    mlp = BanditSklearnModel(window_size=300, action_set=game.action_set, measurement_set=game.measurement_set,
           raw_measurement=False, measurement_to_label=measurement_to_label, finite_measurement=True, policy_map=label_to_action,
          update_frequency=100, model=nn_model)
    mlp.name = 'MLPRegressor_bandit' # <-- add this line
    ...
    import os
    save_dir = f'data/rps_bandit_{M}_{str(measurement_to_label)}'
