...
class SklearnModel(DecisionMaker):
    ...
    def update_energies(self, measurement: Measurement, costs: dict[Action, float], time: float = 0.0, **kwargs
    ):
        """Updates internal model buffers, guarding against None bandit feedback."""
        
        # Guard: Clean the costs dictionary so scikit-learn receives numeric entries
        cleaned_costs = {a: (v if v is not None else 0.0) for a, v in costs.items()}
        
        # Override the original costs variable so the rest of the method uses cleaned data
        costs = cleaned_costs
        if self.measurement_to_label:
...
class BayesianEstimator(DecisionMaker):
    ...
    def update_energies(self, measurement, costs, **kwargs):
        ...
        # FIX: Guard against None values and force float64 type
        cost_arr = np.array([
            (costs[key] if costs[key] is not None else 0.0) for key in self.action_set
        ], dtype=np.float64)
